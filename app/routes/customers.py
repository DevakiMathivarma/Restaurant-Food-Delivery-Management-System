...
from app.auth.permissions import require_admin, require_customer
...
def list_customers(page: int = Query(1, ge=1), limit: int = Query(10, ge=1, le=100), search: str | None = Query(None),
    sort_by: str = Query("created_at"), sort_order: str = Query("desc"), db: Session = Depends(get_db)):
    return get_all_customers(db=db, page=page, limit=limit, search=search, sort_by=sort_by, sort_order=sort_order)


# --- address routes, deliberately placed BEFORE /{customer_id} below -
# "addresses" would otherwise get matched as an attempted customer_id
# integer, and fail, since FastAPI matches routes top to bottom ---
...
# --- generic /{customer_id} routes, deliberately LAST ---
